...
public class TotalNutrientsTest {
    ...
    private TotalNutrients item;

    @Before
    public void init() {
        item = new TotalNutrients();
        ...
        Daily daily26 = new Daily();
        daily26.setLabel("fatrn");
        item.setFatrn(daily26);

        Daily daily27 = new Daily();
        daily27.setLabel("CHOCDFNet");
        item.setCHOCDFNet(daily27);

        Daily daily28 = new Daily();
        daily28.setLabel("fams");
        item.setFams(daily28);

        Daily daily29 = new Daily();
        daily29.setLabel("fams");
        item.setFams(daily29);

        Daily daily30 = new Daily();
        daily30.setLabel("fapu");
        item.setFapu(daily30);

        Daily daily31 = new Daily();
        daily31.setLabel("sugaralcohol");
        item.setSugarAlcohol(daily31);

        Daily daily32 = new Daily();
        daily32.setLabel("water");
        item.setWater(daily32);

        Daily daily33 = new Daily();
        daily33.setLabel("folac");
        item.setFolac(daily33);

        Daily daily34 = new Daily();
        daily34.setLabel("sugar");
        item.setSugar(daily34);

        Daily daily35 = new Daily();
        daily35.setLabel("folfd");
        item.setFolfd(daily35);

        Daily daily36 = new Daily();
        daily36.setLabel("SUGARAdded");
        item.setSUGARAdded(daily36);
    }
    ...
    @Test
    public void getFatrnTest() {
        assertEquals("fatrn", item.getFatrn().getLabel());
    }

    @Test
    public void setFatrnTest() {
        assertEquals("fatrn", item.getFatrn().getLabel());
        Daily daily = new Daily();
        daily.setLabel("fatrn1");
        item.setFatrn(daily);
        assertEquals("fatrn1", item.getFatrn().getLabel());
    }

    @Test
    public void getFamsTest() {
        assertEquals("fams", item.getFams().getLabel());
    }

    @Test
    public void setFamsTest() {
        assertEquals("fams", item.getFams().getLabel());
        Daily daily = new Daily();
        daily.setLabel("fams1");
        item.setFams(daily);
        assertEquals("fams1", item.getFams().getLabel());
    }

    @Test
    public void getFapuTest() {
        assertEquals("fapu", item.getFapu().getLabel());
    }

    @Test
    public void setFapuTest() {
        assertEquals("fapu", item.getFapu().getLabel());
        Daily daily = new Daily();
        daily.setLabel("fapu1");
        item.setFapu(daily);
        assertEquals("fapu1", item.getFapu().getLabel());
    }

    @Test
    public void getCHOCDFNetTest() {
        assertEquals("CHOCDFNet", item.getCHOCDFNet().getLabel());
    }

    @Test
    public void setCHOCDFNetTest() {
        assertEquals("CHOCDFNet", item.getCHOCDFNet().getLabel());
        Daily daily = new Daily();
        daily.setLabel("CHOCDFNet1");
        item.setCHOCDFNet(daily);
        assertEquals("CHOCDFNet1", item.getCHOCDFNet().getLabel());
    }

    @Test
    public void getSugarTest() {
        assertEquals("sugar", item.getSugar().getLabel());
    }
    ...
    public void setSugarTest() {
        assertEquals("sugar", item.getSugar().getLabel());
        Daily daily = new Daily();
        daily.setLabel("sugar1");
        item.setSugar(daily);
        assertEquals("sugar1", item.getSugar().getLabel());
    }

    @Test
    public void getSUGARAddedTest() {
        assertEquals("SUGARAdded", item.getSUGARAdded().getLabel());
    }

    @Test
    public void setSUGARAddedTest() {
        assertEquals("SUGARAdded", item.getSUGARAdded().getLabel());
        Daily daily = new Daily();
        daily.setLabel("SUGARAdded1");
        item.setSUGARAdded(daily);
        assertEquals("SUGARAdded1", item.getSUGARAdded().getLabel());
    }

    @Test
    public void getFolfdTest() {
        assertEquals("folfd", item.getFolfd().getLabel());
    }

    @Test
    public void setFolfdTest() {
        assertEquals("folfd", item.getFolfd().getLabel());
        Daily daily = new Daily();
        daily.setLabel("folfd1");
        item.setFolfd(daily);
        assertEquals("folfd1", item.getFolfd().getLabel());
    }

    @Test
    public void getFolacTest() {
        assertEquals("folac", item.getFolac().getLabel());
    }

    @Test
    public void setFolacTest() {
        assertEquals("folac", item.getFolac().getLabel());
        Daily daily = new Daily();
        daily.setLabel("folac1");
        item.setFolac(daily);
        assertEquals("folac1", item.getFolac().getLabel());
    }

    @Test
    public void getSugarAlcoholTest() {
        assertEquals("sugaralcohol", item.getSugarAlcohol().getLabel());
    }

    @Test
    public void setSugarAlcoholTest() {
        assertEquals("sugaralcohol", item.getSugarAlcohol().getLabel());
        Daily daily = new Daily();
        daily.setLabel("sugaralcohol1");
        item.setSugarAlcohol(daily);
        assertEquals("sugaralcohol1", item.getSugarAlcohol().getLabel());
    }

    @Test
    public void getWaterTest() {
        assertEquals("water", item.getWater().getLabel());
    }
    ...
    public void setWaterTest() {
        assertEquals("water", item.getWater().getLabel());
        Daily daily = new Daily();
        daily.setLabel("water1");
        item.setWater(daily);
        assertEquals("water1", item.getWater().getLabel());
    }
}
